...
select emp_no, first_name, hire_date from employees where hire_date like '%-06-%' or hire_date like  '%-12-%';

-- 4. EMPLOYEES테이블의 레코드 중 사원명에 ‘A’로 시작하고 생일이 1950년~1959년 사이인 사원을 나이가 많은 순으로 선택하라.
select * from employees where first_name like 'A%' and birth_date between '1950-01-01' and
'1959-12-31' order by birth_date asc;
...
select first_name from employees where gender = 'F' and birth_date like '%-01-%' order by first_name asc;

-- 7. EMP테이블의 사원 입사일이 1990-01-01이후인 사원과 이름에 B가 포함된 사원을 입사일 기준 내림차순으로 정렬하여 선택하라.
select * from employees where hire_date>='1990-01-01' && first_name like '%B%' order by
hire_date desc;
...
select emp_no, first_name, hire_date, gender from employees order by first_name asc, hire_date desc;
